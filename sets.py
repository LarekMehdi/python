...
def tag_special_ingredients(dish):
    ...
    set_ingredient = set(specials)
    return name, set_ingredient
...
def separate_appetizers(dishes, appetizers):
    ...
    dedup_dishes = set(dishes)
    dedup_appetizers = set(appetizers)
    dedup_dishes = dedup_dishes - dedup_appetizers

    return list(dedup_dishes)
...
def singleton_ingredients(dishes, intersection):
    ...
    for dishe in dishes:
        for ingredient in dishe:
            if ingredient in dic.keys():
                dic[ingredient] = dic[ingredient] + 1
            else:
                dic[ingredient] = 1
